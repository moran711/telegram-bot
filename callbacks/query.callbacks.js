...
const {formatDaysForKeyboard, days} = require("../helpers/formatDaysForKeyboard");
...
const sceduleQuery = (bot) => (async (query) => {
  ...
  if (await instituteController.getInstituteById(selectedItem)) {
    ...
    bot.sendMessage(chatId, "Виберіть день тижня для групи", {reply_markup: {inline_keyboard: formatDaysForKeyboard(scedule[0]._id)}});
  } else if(await sceduleController.getSceduleById(possibleSceduleId) && Object.keys(days).includes(possibleSceduleDay) ) {
    const scedule = await sceduleController.getSceduleById(possibleSceduleId);

    const sceduleMarkdown = scedule.week[possibleSceduleDay][getTypeOfWeek].map(couple => formatSceduleForСouple(couple)).join('\n--------------\n');
    sceduleMarkdown ? bot.sendMessage(chatId, sceduleMarkdown, {parse_mode: "html"})
